...
function bumpPackageVersion {
    if [ $(matchCommitMessagePattern "build|chore|ci|docs|fix|perf|refactor|style|test") != "0" ]; then
        ...
        echo "Commit message should start with one of the following types, please see https://github.com/lazycuh/angular-anchored-floating-box/blob/main/CONTRIBUTING.md to learn more"
        npm version minor --no-git-tag-version
    fi
